...
This document consolidates research findings for technology choices, implementation patterns, and best practices for the AIWebFeeds platform. All decisions are informed by the constitution principles, specification requirements, and clarification session outcomes.
...
**Rationale**:
- **feedparser** (v6.0+): Industry-standard Python library for RSS/Atom/JSON Feed parsing
...
# Metrics
validation_counter = Counter('feed_validations_total', 'Total feed validations', ['status'])
validation_duration = Histogram('feed_validation_duration_seconds', 'Validation duration')
...
def generate_opml(feeds: list[FeedSource], title: str = "AI Web Feeds") -> str:
    ...
    ET.SubElement(head, "dateCreated").text = datetime.now(UTC).strftime("%a, %d %b %Y %H:%M:%S GMT")
...
**Rationale**:
- **Build-Time Integration**: Python CLI generates JSON files, Next.js imports during build
...
- **Build Process**: Vercel/GitHub Actions
  ...
  2. Run `pnpm build` in `apps/web/` (Next.js build consumes JSON)
  3. Deploy static site + API routes
        ...
        "feeds": [
            {
                ...
                "lastValidated": feed.last_validated.isoformat() if feed.last_validated else None,
            }
            for feed in feeds
        ],
        "meta": {
...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="sqlite:///data/aiwebfeeds.db",
...
def process_all_feeds():
    ...
    with tqdm(total=len(categories), desc="Processing categories", position=0) as category_bar:
        for category in categories:
            feeds = get_feeds_by_category(category)
            
            with tqdm(total=len(feeds), desc=f"  {category.title()}", position=1, leave=False) as feed_bar:
                for feed in feeds:
                    process_feed(feed)
                    feed_bar.update(1)
...
2. `contracts/`: OpenAPI spec, JSON schemas
3. `quickstart.md`: Setup guide, development workflow
