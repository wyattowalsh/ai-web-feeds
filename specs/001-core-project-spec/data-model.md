...
This document defines the complete data model for AIWebFeeds, including entity schemas,
relationships, validation rules, and state transitions. All entities use Pydantic v2 +
SQLModel for type safety and database mapping. Configuration management uses
...
**Purpose**: Represents an individual RSS/Atom/JSON Feed with metadata, validation
status, and topics.
...
class FeedSource(SQLModel, table=True):
    ...
    )  # e.g., "openai-blog", "arxiv-cs-lg"
    url: str = Field(
        unique=True, index=True, max_length=500
    ...
    feed_format: str | None = Field(
        default=None, max_length=20
    )  # rss2.0, atom1.0, jsonfeed1.1
...
**Purpose**: Hierarchical taxonomy node for categorizing feeds by AI/ML domains, tasks,
...
class Topic(SQLModel, table=True):
    ...
    facet_group: str | None = Field(
        default=None, max_length=50
    )  # conceptual, technical, etc.
    ...
    i18n: dict[str, dict[str, str]] = Field(
        default_factory=dict, sa_column_kwargs={"type_": "JSON"}
    )
...
**Purpose**: Directed or symmetric relationships between topics forming the taxonomy
graph.
...
class ValidationResult(SQLModel, table=True):
    ...
    # Parsing results
    format_detected: str | None = Field(
        default=None, max_length=20
    )  # rss2.0, atom1.0, etc.
...
**Purpose**: AI-inferred metadata including topics, quality scores, and source type
classification.
...
class EnrichmentMetadata(SQLModel, table=True):
    ...
    # Metadata
    enrichment_method: str | None = Field(
        default=None, max_length=100
    )  # "content-analysis", "manual", "llm"
...
def calculate_quality_score(feed: FeedSource, entries: list[FeedEntry]) -> float:
    ...
    if entries:
        recent_entries = [
            e
            for e in entries
            if e.published and (datetime.now(UTC) - e.published).days < 30
        ]
        frequency_score = min(
            len(recent_entries) / 10.0, 1.0
        )  # 10+ entries/month = 1.0
...
- `ContributionSubmission`: id (PK), feed_source_id (FK), contributor_github, status,
  submitted_at
...
- `data/feeds.enriched.yaml` → `data/feeds.enriched.schema.json` (FeedSource +
  EnrichmentMetadata)
