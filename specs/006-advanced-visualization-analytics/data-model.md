...
This document defines the SQLAlchemy models and database schema for Phase 006
visualization features. All entities use device-based identification (no user accounts)
via `device_id` column populated from browser localStorage UUID.
...
class Visualization(Base):
    ...
    chart_type = Column(
        String(50), nullable=False, comment="line|bar|scatter|pie|area|heatmap"
    )
    data_source = Column(
        String(50),
        nullable=False,
...
class DashboardWidget(Base):
    ...
    visualization_id = Column(
        Integer, ForeignKey("visualizations.id", ondelete="SET NULL"), nullable=True
    )

    # Widget configuration
    widget_type = Column(
        String(50), nullable=False, comment="chart|metric_card|feed_list|topic_cloud"
    )
    ...
    position = Column(
        JSON, nullable=False, comment="{x, y, w, h} for React Grid Layout"
    )
...
class Forecast(Base):
    ...
    model_type = Column(
        String(50), nullable=False, default="prophet", comment="prophet|arima|lstm"
    )
    ...
    model_params = Column(
        JSON, nullable=False, comment="Model hyperparameters and settings"
    )
...
class APIKey(Base):
    ...
    key_hash = Column(
        String(60), nullable=False, unique=True, comment="bcrypt hash of API key"
    )
...
def upgrade():
    ...
    op.create_index(
        "idx_viz_device_created", "visualizations", ["device_id", "created_at"]
    )
    op.create_index(
        "idx_viz_device_viewed", "visualizations", ["device_id", "last_viewed"]
    )
    ...
    op.create_index(
        "idx_dashboard_device_updated", "dashboards", ["device_id", "updated_at"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["visualization_id"], ["visualizations.id"], ondelete="SET NULL"
        ),
    )
    ...
    op.create_index(
        "idx_forecast_topic_generated", "forecasts", ["topic_id", "generated_at"]
    )
    ...
    op.create_index(
        "idx_apikey_device_created", "api_keys", ["device_id", "created_at"]
    )
    ...
    op.create_index(
        "idx_exportjob_device_status", "export_jobs", ["device_id", "status"]
    )
    ...
    op.create_index(
        "idx_apiusage_key_timestamp", "api_usage", ["api_key_id", "timestamp"]
    )
