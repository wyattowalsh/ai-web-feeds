...
This document defines the complete data model for Phase 2 features: Analytics Dashboard,
Intelligent Search & Discovery, and AI-Powered Feed Recommendations. All entities use
SQLModel for type-safe database interactions and Pydantic v2 for validation.
...
**Purpose**: Represents a feed source with enhanced fields for search and
recommendations.
...
class FeedSource(SQLModel, table=True):
    ...
    )  # Computed: follower count, click-through rate
...
- `source_type` MUST be one of: `blog`, `podcast`, `newsletter`, `video`, `social`,
  `other`
...
**Purpose**: Stores aggregated analytics metrics at a point in time for historical
trending.
...
class AnalyticsSnapshot(SQLModel, table=True):
    ...
    trending_topics: str = Field(
        sa_column_kwargs={"type_": JSON}
    )  # JSON array: [{"topic": "LLM", "count": 45}, ...]
    health_distribution: str = Field(
        sa_column_kwargs={"type_": JSON}
    )  # JSON object: {"healthy": 800, "moderate": 150, "unhealthy": 50}
    created_at: datetime = Field(default_factory=datetime.utcnow)
```
...
snapshot = AnalyticsSnapshot(
    ...
    health_distribution=json.dumps({"healthy": 800, "moderate": 150, "unhealthy": 50}),
)
```
...
class SearchQuery(SQLModel, table=True):
    ...
    clicked_results: str | None = Field(
        default=None, sa_column_kwargs={"type_": JSON}
    )  # JSON array of feed_ids
...
class SavedSearch(SQLModel, table=True):
    ...
    user_id: str = Field(
        index=True
    )  # Required (anonymous user ID or authenticated user ID)
...
class RecommendationInteraction(SQLModel, table=True):
    ...
    context: str | None = Field(
        default=None, sa_column_kwargs={"type_": JSON}
    )  # JSON: {"recommendation_algorithm": "content_based", "score": 0.85}
    timestamp: datetime = Field(default_factory=datetime.utcnow, index=True)
```

**Validation Rules**:

- `interaction_type` MUST be one of: `impression`, `click`, `like`, `dismiss`,
  `block_topic`
...
interaction = RecommendationInteraction(
    ...
    context=json.dumps(
        {"algorithm": "content_based", "score": 0.85, "topics": ["LLM", "NLP"]}
    ),
)
```
...
**Purpose**: Stores user preferences and interests (localStorage in Phase 2, database in
Phase 3).
...
class UserProfile(SQLModel, table=True):
    ...
    followed_feeds: str = Field(
        default="[]", sa_column_kwargs={"type_": JSON}
    )  # JSON array of feed_ids
    preferred_topics: str = Field(
        default="[]", sa_column_kwargs={"type_": JSON}
    )  # JSON array of topics
    blocked_topics: str = Field(
        default="[]", sa_column_kwargs={"type_": JSON}
    )  # JSON array of blocked topics
...
def upgrade():
    ...
    op.add_column(
        "sources", sa.Column("last_validated_at", sa.DateTime(), nullable=True)
    )
    ...
    op.create_index(
        "idx_sources_active_verified",
        "sources",
        ["is_active", "verified", "popularity_score"],
    )
    ...
...
**New Tables**: 8 (FeedEmbedding, AnalyticsSnapshot, TopicStats, SearchQuery,
SavedSearch, RecommendationInteraction, UserProfile, CollaborativeMatrix)\
**Extended Tables**: 1 (FeedSource with popularity_score, last_validated_at,
validation_count)\
...
**Configuration Classes**: 4 (EmbeddingSettings, AnalyticsSettings, SearchSettings,
