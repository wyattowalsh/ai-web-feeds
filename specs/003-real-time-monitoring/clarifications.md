...
This document identifies ambiguities in the Phase 3B specification and provides
clarifying questions to resolve them before implementation planning.
...
**Ambiguity**: Spec assumes "user_id" in database tables, but Phase 3A (User Accounts)
is not a dependency.

**Question**: How do we identify users for notification preferences without
authentication?

- **Option A**: Use browser localStorage with generated UUID (anonymous users, data not
  portable)
...
**Recommendation**: **Option A** for MVP - localStorage UUID. Migrate to Phase 3A
accounts later.

**Impact**: Medium - Affects notification_preferences table, API authentication, data
portability
...
**Ambiguity**: User Story 1 mentions "feeds I follow" but Phase 2 doesn't implement
persistent "follow" functionality.
...
**Recommendation**: **Option B** - Server-side storage with localStorage-based user_id
for reliability.
...
**Ambiguity**: Spec mentions two options: Celery+Redis vs APScheduler, but doesn't
recommend one.

**Question**: Which background job system should we use?

- **Option A**: **APScheduler** (MIT, in-process, simpler, no Redis required)
- **Option B**: **Celery + Redis** (distributed, scalable, adds infrastructure
  complexity)

**Recommendation**: **Option A (APScheduler)** for MVP. Redis only needed at >1000
concurrent WebSocket connections (not MVP scale).
...
**Ambiguity**: With 10,000 feeds and 30-minute average interval, how do we prioritize
...
- **Option C**: Dynamic priority based on update frequency (feeds that update often get
  priority)

**Recommendation**: **Option C** - Dynamic priority based on historical update
frequency.
...
**Ambiguity**: FR-017 mentions browser push notifications, but spec doesn't detail
service worker setup.

**Question**: Do we implement browser push notifications (Web Push API) in Phase 3B?

- **Option A**: **Yes** - Full Web Push API with service worker (notifications when tab
  closed)
- **Option B**: **No** - In-app notifications only (simpler, notifications only when app
  open)
- **Option C**: **Defer to Phase 4** - Focus on WebSocket + email digests first

**Recommendation**: **Option B** for MVP - In-app notifications only. Web Push API in
Phase 4.
...
**Ambiguity**: FR-016 says "persist notifications for 7 days" but edge cases mention
...
**Recommendation**: **Option A** - Hard delete after 7 days. Disk space is cheap, but
queries get slower with millions of rows.
...
**Ambiguity**: Spec mentions SendGrid free tier (100 emails/day) but doesn't address
scaling beyond that.
...
**Recommendation**: **Option A** - Self-hosted SMTP for production, SendGrid for
dev/staging.
...
**Ambiguity**: Spec mentions MJML for email templates but doesn't specify template
structure.

**Question**: How complex should email digests be?

- **Option A**: Simple HTML with table of articles (fast to implement, limited design)
- **Option B**: MJML responsive templates with charts, images, branding (polished but
  complex)
...
**Ambiguity**: FR-021 says "compare against 7-day baseline" but doesn't specify how to
compute baseline for new topics.
...
**Recommendation**: **Option C** - 3-day minimum baseline. Enables faster trending
detection for emerging topics.
...
**Ambiguity**: FR-028 mentions "regex support" for keyword alerts, but regex can be
dangerous (ReDoS attacks).

**Question**: Do we allow arbitrary regex patterns?

- **Option A**: **No regex** - Simple case-insensitive substring matching only
- **Option B**: **Limited regex** - Whitelist safe patterns (no lookaheads,
  backtracking)
- **Option C**: **Full regex** - Allow any pattern with timeout protection

**Recommendation**: **Option A** for MVP - Simple substring matching. Regex in Phase 4
...
**Ambiguity**: FR-003 says "store last 100 entries per feed" but doesn't specify what
happens to older entries.
...
**Recommendation**: **Option C** - Store all entries. SQLite handles millions of rows
efficiently. Disk is cheap.
...
**Ambiguity**: Spec says SQLite handles "10k feeds with 100k articles" but doesn't
specify max database size.
...
**Recommendation**: Yes - 2GB SQLite database is well within limits (SQLite supports up
to 281TB). Monitor and optimize indexes.
...
**Ambiguity**: NFR-003 says "handle 1000 concurrent WebSocket connections" but doesn't
specify single vs multi-instance.

**Question**: At what point do we scale WebSocket server horizontally?

- **Option A**: Single instance handles all connections (simpler, requires Redis pub/sub
  at >1000)
- **Option B**: Multi-instance from start (complex, enables unlimited scale)

**Recommendation**: **Option A** - Single instance for MVP. Add Redis pub/sub when
approaching 1000 concurrent connections.
...
**Ambiguity**: Spec says "poll 10,000 feeds within 1 hour" but doesn't specify
concurrency.
...
**Recommendation**: Start with **10 parallel workers** (httpx async). Monitor
CPU/network usage, scale up if needed.
...
1. **Q2**: Followed feeds storage mechanism
1. **Q3**: Background job queue selection (APScheduler vs Celery)
1. **Q8**: SendGrid scaling strategy

**High (Should resolve before Phase 2 - Design)**: 5. **Q4**: Feed polling
prioritization algorithm 6. **Q6**: Browser Push API scope (defer or include) 7.
**Q10**: Trending baseline for new topics

**Medium (Can resolve during implementation)**: 8. **Q5**: WebSocket implementation
(Socket.IO vs native) 9. **Q7**: Notification retention policy 10. **Q9**: Email
template complexity 11. **Q11**: Keyword alert regex support 12. **Q12**: Feed entry
storage limit

**Low (Optimization decisions)**: 13. **Q13**: SQLite database size monitoring 14.
**Q14**: WebSocket horizontal scaling trigger 15. **Q15**: Feed polling parallelization
tuning
...
1. **Followed Feeds**: Server-side storage with localStorage user_id
1. **Background Jobs**: APScheduler (no Redis for MVP)
1. **Email Delivery**: Self-hosted SMTP (production) + SendGrid (dev)
1. **WebSocket**: Socket.IO (mature, auto-fallback)
1. **Browser Push**: **Defer to Phase 4** (focus on in-app + email)
1. **Trending Baseline**: 3-day minimum for new topics
1. **Feed Entry Limit**: Store all entries (no limit)
1. **Notification Retention**: Hard delete after 7 days
1. **Email Templates**: Simple HTML (no MJML for MVP)
