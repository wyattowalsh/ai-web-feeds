...
This document defines the complete data model for Phase 3B features: Feed Monitoring, Real-Time Notifications, Trending Topics, and Email Digests. All entities use SQLModel for type-safe database interactions and Pydantic v2 for validation.
...
**Purpose**: Stores discovered articles from feed polling for notification and trending detection.
...
class FeedEntry(SQLModel, table=True):
    ...
    content_html: Optional[str] = Field(default=None)  # Full article content if available
    pub_date: datetime = Field(index=True)
    author: Optional[str] = Field(default=None, max_length=255)
    categories: Optional[str] = Field(default=None)  # JSON array: ["LLM", "Computer Vision"]
...
**Purpose**: Tracks scheduled and completed feed polling jobs for monitoring and debugging.
...
**Purpose**: Stores user notifications for WebSocket delivery and in-app notification center.
...
class Notification(SQLModel, table=True):
    ...
    metadata: Optional[str] = Field(default=None)  # JSON: {feed_id, article_id, trend_score}
...
**Purpose**: Stores detected trending topics with z-score and related metadata for alerts.
...
**Purpose**: Stores user notification settings (frequency, delivery method, quiet hours).
...
class NotificationPreference(SQLModel, table=True):
    ...
    feed_id: Optional[str] = Field(default=None, foreign_key="feeds.id")  # NULL = global
    ...
    __table_args__ = (
        UniqueConstraint("user_id", "feed_id", "delivery_method", name="unique_user_feed_method"),
    )
...
class EmailDigest(SQLModel, table=True):
    ...
    timezone: str = Field(default="UTC", max_length=50)  # IANA timezone: "America/Los_Angeles"
