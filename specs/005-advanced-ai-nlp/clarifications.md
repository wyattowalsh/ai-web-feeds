...
### Core Principle
**ALL NLP operations run as batch jobs** → Process articles from `feed_entries` table → Write results to SQLite tables → No real-time API dependencies.
...
**Implementation**: APScheduler cron jobs in `packages/ai_web_feeds/src/ai_web_feeds/nlp_scheduler.py`
...
- **Alternative**: `cardiffnlp/twitter-roberta-base-sentiment` (125MB, better for short text)
...
- **Why LDA over BERTopic**: BERTopic requires sentence-transformers (500MB+), LDA is lightweight
- **Fallback**: Simple TF-IDF + K-Means clustering

**Model Caching**: Download models to `~/.cache/ai_web_feeds/models/` on first run, reuse indefinitely.
...
2. Run NLP model (quality/entities/sentiment)
3. Write results to corresponding table (`article_quality_scores`, `entities`, etc.)
4. Set `*_processed = TRUE` + timestamp
...
- **Manual Aliases**: 
  - Store common aliases in `entities.aliases` JSON field: `["G. Hinton", "Prof. Hinton"]`
...
2. For each article's topics (from `topics.yaml` matching):
   - Aggregate sentiment by (topic, date)
   - Compute: avg, positive/neutral/negative counts
3. Upsert into `topic_sentiment_daily` table
...
- **Entity Extraction**: If spaCy fails → Use rule-based fallback → If that fails, skip entity processing
- **Sentiment Analysis**: If model fails → Mark as "unavailable", don't create `article_sentiment` record
...
- ✅ **Model Accuracy**: Sentiment classification accuracy ≥75% (manual spot-check of 100 articles)
...
- 📊 **Quality Filter Adoption**: 40% of search queries use quality filter (defer until search UI updated)
- 📊 **Entity Page Views**: 30% of users click on entity at least once (defer until entity pages built)
- 📊 **Sentiment Dashboard**: 20% of users view sentiment trends monthly (defer until dashboard built)

**Acceptance Criteria for Phase 5**:
1. ✅ All 4 batch jobs (quality, entities, sentiment, topics) running on schedule
2. ✅ Process 1000 articles → 95% success rate, <5% failures
3. ✅ SQLite tables populated with results (quality scores, entities, sentiment)
4. ✅ CLI commands work (`aiwebfeeds nlp stats`, `list-entities`, etc.)
5. ✅ Unit + integration tests pass with ≥90% coverage

**Phase 6 (Future)**: Build UI to surface NLP results (quality filtering, entity pages, sentiment dashboard)
