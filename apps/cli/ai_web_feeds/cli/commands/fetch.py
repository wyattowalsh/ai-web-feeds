...
from ai_web_feeds.cli.support import CommandResult, ExitCode, ResultFormat, console, render_result
from ai_web_feeds.config import resolve_runtime_database_url
from ai_web_feeds.enrich import AdvancedEnricher
from ai_web_feeds.models import FeedFetchLog, FeedItem
...
@app.command("one")
def fetch_one(
    feed_id: str = typer.Argument(..., help="Stored feed ID to fetch"),
...
def fetch_all(
    ...
    result = CommandResult(
        ...
        summary="Completed stored feed fetch run",
        details={
