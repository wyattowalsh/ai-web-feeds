...
@app.command("topics")
def validate_topics(
    topics_file: Optional[Path] = typer.Option(None, "--file", "-f", help="Path to topics.yaml file"),
    schema_file: Optional[Path] = typer.Option(None, "--schema", "-s", help="Path to schema file"),
):
...
def validation_report(
    ...
    for data in health_data[:20]:  # Top 20
        health_color = "green" if data["health"] >= 0.8 else "yellow" if data["health"] >= 0.5 else "red"
    ...
    console.print(f"Healthy Feeds (≥0.8): {healthy_feeds} ({healthy_feeds/len(health_data)*100:.1f}%)")
