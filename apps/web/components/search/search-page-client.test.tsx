...
describe("SearchPageClient", () => {
  ...
  it("updates the URL and runs a search when a new query is submitted", async () => {
    ...
    await waitFor(() => {
      expect(pushMock).toHaveBeenCalledWith("/feeds?q=fresh+query&scope=sources");
    });
    await waitFor(() => {
  ...
  it("reruns the active query when the scope changes", async () => {
    ...
    await waitFor(() => {
      expect(pushMock).toHaveBeenCalledWith("/feeds?q=agents&scope=articles");
    });
  ...
  it("preserves explicit feed slices across follow-up article searches", async () => {
    ...
    await waitFor(() => {
      expect(pushMock).toHaveBeenCalledWith(
        "/feeds?q=fresh+query&scope=articles&feed=feed-2&feed=feed-1",
      );
    });
