...
describe("ReaderPageClient", () => {
  ...
  it("honors root reader filters and reader-specific query state", () => {
    currentPathname = "/";
    currentSearchParams = new URLSearchParams(
    ...
    expect(screen.getByRole("link", { name: "Catalog" })).toHaveAttribute(
      "href",
      "/reader?source_type=blog&topics=agents&verified=true&q=roundup&reader_view=bookmarked&reader_sort=source&stream=all&cursor=24",
    );
    expect(screen.getByRole("link", { name: "Articles" })).toHaveAttribute(
      "href",
      "/reader?source_type=blog&topics=agents&verified=true&q=roundup&reader_view=bookmarked&reader_sort=source&stream=all&cursor=24&mode=articles",
    );
    ...
    expect(replaceMock).toHaveBeenLastCalledWith(
      "/reader?source_type=blog&topics=agents&verified=true&q=roundup&reader_view=bookmarked&reader_sort=oldest&stream=all&cursor=24",
      { scroll: false },
    );
  });
  ...
  it("keeps the embedded root reader surface compact and canonical", () => {
    currentPathname = "/";
    ...
    expect(screen.getByRole("link", { name: "Catalog" })).toHaveAttribute(
      "href",
      "/reader?feed=feed-1&source_type=blog&q=roundup",
    );
    expect(screen.getByRole("link", { name: "Articles" })).toHaveAttribute(
      "href",
      "/reader?feed=feed-1&source_type=blog&q=roundup&mode=articles",
    );
  });
