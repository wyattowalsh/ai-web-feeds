...
3. Export all pages to PDF
4. Automatically shut down the server
...
3. In a new terminal, run the export:
   ```bash
   pnpm export-pdf
   ```
...
The `mdx-components.tsx` file contains print-friendly versions of interactive components like Accordions and Tabs. When `NEXT_PUBLIC_PDF_EXPORT=true` is set, these components render all content in an expanded state.

## ⚙️ Configuration

### Environment Variables

- `NEXT_PUBLIC_PDF_EXPORT`: Set to `'true'` to enable PDF-friendly rendering of interactive components
...
- The export process automatically discovers all documentation pages from the Fumadocs source
