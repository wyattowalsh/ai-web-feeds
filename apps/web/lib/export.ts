...
export async function exportCSV(): Promise<Blob> {
  ...
  const rows = allArticles.map((article) => [
    ...
    article.pubDate === null ? "" : new Date(article.pubDate).toISOString(),
...
export async function exportHTML(): Promise<Blob> {
  ...
  allArticles
    ...
    .forEach((article) => {
      ...
      html.push("        <div>");
      html.push(
        `          <div class="article-title"><a href="${escapeHtml(
          article.link,
        )}" target="_blank">${escapeHtml(article.title)}</a></div>`,
      );
      html.push(
        `          <div class="article-meta">${publishedLabel} ${
  ...
  html.push(
    "    <p>This is an offline archive. All functionality works without internet connection.</p>",
  );
  html.push("  </footer>");
  html.push("");
  html.push("  <script>");
