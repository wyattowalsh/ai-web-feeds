...
export const preferences = {
  async get(): Promise<Preferences> {
    const prefs = await get<Partial<Preferences>>(STORES.PREFERENCES, DEFAULT_PREFERENCES.id);
    const normalized = normalizePreferences(prefs);

    if (preferencesNeedMigration(prefs)) {
      await put(STORES.PREFERENCES, normalized);
    }
