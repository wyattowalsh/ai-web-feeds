...
# apps/web is a nested pnpm workspace root with its own pnpm-lock.yaml
# Run web package-manager commands from this directory unless a doc explicitly says otherwise.
...
│   └── ai-web-feeds.db       # SQLite database (legacy aiwebfeeds.db still supported)
...
# Test commands
uv run ai-web-feeds fetch url "https://example.com/feed"
uv run ai-web-feeds validate all
uv run ai-web-feeds export json --input data/feeds.enriched.yaml --output reports/feeds.json
```
...
- [ ] `/llms.txt`, `/llms-full.txt`, and markdown route examples checked if docs/navigation changed
- [ ] Pre-commit hooks pass
- [ ] No new linting warnings
- [ ] Type hints added
- [ ] CHANGELOG.md updated (if significant)

For the docs-specific flow, see [Docs Maintenance Checklist](/docs/contributing/docs-maintenance).
...
- **LLM output:** Keep frontmatter summaries, `meta.json`, `/llms.txt`, and `/llms-full.txt` aligned

### Testing

- **Framework:** Pytest (Python), Vitest + Playwright (TypeScript/web)
...
# Commitizen
pnpm exec cz              # Interactive commit
git cz                    # Alternative

# Commitlint
pnpm exec commitlint --from HEAD~1   # Validate last commit
echo "msg" | pnpm exec commitlint    # Test message
```
...
See [Workflow Reference](/docs/guides/workflow-reference) for the current release and automation entrypoints.
