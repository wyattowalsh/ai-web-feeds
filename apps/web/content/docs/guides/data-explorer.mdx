...
The Data Explorer at `/explorer` is an advanced map surface for browsing the AI Web Feeds taxonomy and feed relationships. Use `/feeds` for the canonical reader, switch to `/feeds?mode=catalog` for source refinement, and open `/explorer` when you need structural context rather than a reading-first workflow.
