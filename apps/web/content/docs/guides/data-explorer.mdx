---
title: Data Explorer
description: Legacy taxonomy map reference; the public explorer route now redirects to the compact dashboard.
---

# Data Explorer

The former Data Explorer at `/explorer` has been removed from the public UI
path and now redirects to `/dashboard`. Use `/` for the reader,
`/?mode=catalog` for source refinement, and repo data files or docs when you
need taxonomy details.
...
1. Open `/dashboard` for collection health.
2. Inspect `data/topics.yaml` and `data/feeds.yaml` for taxonomy details.
