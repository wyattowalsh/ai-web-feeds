...
The Data Explorer at `/explorer` is a secondary map surface for browsing the AI Web Feeds taxonomy and feed relationships. Use `/` for the canonical reader, switch to `/?mode=catalog` for source refinement, and open `/explorer` when you need to inspect structure rather than read recent posts.
