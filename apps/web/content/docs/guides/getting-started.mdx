...
Once the project is running, start at `/feeds` for the canonical reader. Switch to
`/feeds?mode=catalog` for source refinement, treat `/` as a compatibility redirect
