...
- `NEXT_PUBLIC_BASE_URL`, `NEXT_PUBLIC_WEBSOCKET_URL`, and
  `NEXT_PUBLIC_PDF_EXPORT` are build-time values for the Next.js app. Set them
  before `docker compose build` when you need non-default values.
...
# Initialize runtime search tables (optional for CLI/runtime workflows)
...
# Generate embeddings (optional, for runtime semantic search experiments)
uv run aiwebfeeds search embeddings --provider local
```

### 4. Verify Runtime Workflows
...
These commands are useful when you are running the Python runtime and its storage
features. They are **not** required for the default public web loop:
...
For the default deployment, the important point is that `BACKEND_URL` is optional.
The primary public read-only surfaces work without it:
...
Only backend-dependent personalized or storage-backed routes need `BACKEND_URL`,
for example:
...
- **Migration Path**: SQLModel supports both, change `AIWF_DATABASE_URL`
...
cp data/backups/ai-web-feeds-YYYYMMDD.db.gz data/
gunzip data/ai-web-feeds-YYYYMMDD.db.gz
mv data/ai-web-feeds-YYYYMMDD.db data/ai-web-feeds.db
