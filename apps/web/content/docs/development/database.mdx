...
`DatabaseManager.create_db_and_tables()` is the supported bootstrap entrypoint.

- File-backed runtime databases apply Alembic revisions from `packages/alembic/`.
- `sqlite:///:memory:` remains a test-only fast path that uses
  `SQLModel.metadata.create_all()`.
- `data/ai-web-feeds.db` is the canonical SQLite filename.
- `data/aiwebfeeds.db` is a legacy fallback that is only used when the canonical
  file is absent.
...
For application bootstrap, prefer `create_db_and_tables()` over ad-hoc SQL or
manual table creation so file-backed environments stay aligned with Alembic.
...
```bash
uv run alembic -c packages/alembic.ini revision --autogenerate -m "Initial schema"
```
...
```bash
# Upgrade to latest
uv run alembic -c packages/alembic.ini upgrade head

# Downgrade one version
uv run alembic -c packages/alembic.ini downgrade -1

# Show current version
uv run alembic -c packages/alembic.ini current
```
