...
> **Note**: The `/for-you` personalized recommendations page requires the `BACKEND_URL` environment variable to be set (pointing to a running ai-web-feeds backend). Without it, the route shows an intentional empty state / explanatory message. See [runtime-authority](/docs/development/runtime-authority) and [recommendations](/docs/features/recommendations) docs.
...
uv run ai-web-feeds monitor start
...
uv run ai-web-feeds monitor follow <user-id> <feed-id>

# Example:
uv run ai-web-feeds monitor follow a1b2c3d4-... ai-news

# List your follows
uv run ai-web-feeds monitor list-follows <user-id>

# Unfollow
uv run ai-web-feeds monitor unfollow <user-id> <feed-id>
```
...
User-source relationships for notification targeting:
...
// Body
{
  ...
  "user_id": "<uuid>" // or pass user_id in the query string
}
```
...
AIWF_WEBSOCKET_CORS_ORIGINS=http://localhost:3000,https://aiwebfeeds.w4w.dev
# Optional:
# - Local dev defaults to http://localhost:8000
# - Non-local deployments default to same-origin unless overridden
# NEXT_PUBLIC_WEBSOCKET_URL=https://monitor.example.com

# Backend proxy for web app features (required for /for-you, recommendations, analytics proxies)
BACKEND_URL=http://localhost:8001  # or http://127.0.0.1:8001 ; without this, /for-you intentionally shows empty/intent state
...
1. `articles` - Article metadata from polling
2. `feed_poll_jobs` - Polling job tracking
3. `notifications` - User notifications
4. `user_source_follows` - Source follow relationships
...
```bash
uv run ai-web-feeds monitor start [--port 8000]
```

**Check status**:

```bash
uv run ai-web-feeds monitor status
```
...
1. Check server is running: `uv run ai-web-feeds monitor status`
...
2. Confirm follow relationship: `ai-web-feeds monitor list-follows <user-id>`
...
1. Check scheduler: `ai-web-feeds monitor status`
