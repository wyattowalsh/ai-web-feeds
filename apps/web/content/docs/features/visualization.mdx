...
<Callout type="info">These pages proxy to backend routes under `/api/v1/*`. Without a configured backend they return a `503 FEATURE_UNAVAILABLE` response and the UI surfaces remain usable with sample or local data where applicable.</Callout>
