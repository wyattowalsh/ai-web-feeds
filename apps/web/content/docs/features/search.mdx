...
description: Reader article browse plus catalog discovery inside the primary /feeds workspace with /feeds?mode=catalog for source refinement
---
...
The Search & Discovery experience lives inside the primary reader route `/feeds`
and its explicit catalog submode `/feeds?mode=catalog`. It answers two different
...
`/feeds` is the default reader route.

- `/feeds` opens the article workspace
- `/feeds?mode=catalog` opens the explicit source picker
- `/` remains available as a compatibility redirect into the same workspace state
...
4. Switch to `/feeds?mode=catalog` when you need to change the source list
...
- ✅ Reader `/feeds` works without a required Python HTTP backend
