...
description: Reader article browse plus catalog discovery inside the primary / workspace with /?mode=catalog for source refinement
---
...
The Search & Discovery experience lives inside the primary reader route `/`
...
`/` is the default reader route.

- `/` opens the article workspace
- `/?mode=catalog` opens the explicit source picker
- `/feeds` remains available as a compatibility redirect into the same workspace state
...
4. Switch to `/?mode=catalog` when you need to change the source list
...
- ✅ Reader `/` works without a required Python HTTP backend
...
- ✅ `/` remains available as a compatibility redirect
