...
description: Backend recommendation APIs retained for integrations; the public page redirects to the reader.
---

Recommendations are no longer a standalone public UI surface. `/recommendations`
redirects to the reader at `/`; backend recommendation APIs remain available
for integrations when `BACKEND_URL` points to a running `ai_web_feeds.web_api`
service.
...
- `/recommendations` still redirects to `/`
