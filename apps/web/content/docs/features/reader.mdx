---
title: Reader
description: Reader-first AI feeds workspace backed by a generated article corpus with optional live refresh overlays
---
...
The reader is now the default surface for AI Web Feeds. It uses the generated
...
- **Reader-first default** opens the article workspace immediately on `/feeds`.
- **Catalog mode** remains available as `/feeds?mode=catalog` when you want to
  change the source slice.
- **Corpus-backed article list** reads normalized article rows from
  `data/articles.generated.json`.
- **Preview pane** shows summary/content, metadata, and direct article actions.
...
the user explicitly refreshes the current slice.
