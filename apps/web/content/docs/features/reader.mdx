...
> **Status**: ✅ Available
> **Surface**: `/` > **Catalog submode**: `/?mode=catalog` > **Compatibility redirect**: `/feeds`
...
`/feeds` redirects into the same workspace for compatibility, but new docs and links should point to `/` unless backward compatibility is the point.
...
`/`; `/feeds` is the compatibility redirect.
