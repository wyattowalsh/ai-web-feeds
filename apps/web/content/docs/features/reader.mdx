---
title: Reader
description: Canonical feeds reader backed by a generated article corpus with /?mode=catalog source refinement and optional live refresh overlays
---

`/` is the main workspace in AI Web Feeds. It combines a generated article library with the catalog filters from the curated source list.

> **Status**: ✅ Available
> **Surface**: `/` > **Catalog submode**: `/?mode=catalog`
...
- **Reader default** opens the article workspace on `/`.
...
There is no separate `/feeds` page. New docs and links should point to `/` or
`/?mode=catalog`.
...
app server to fetch recent posts from the current source slice through
...
- the live fallback checks all matching sources for the current slice
...
If both the generated corpus and live feed fallback are unavailable, the reader
shows a compact recovery state with source browsing and retry actions.
