---
title: Reader
description: Canonical feeds reader backed by a generated article corpus with /feeds?mode=catalog source refinement and optional live refresh overlays
---
...
> **Status**: ✅ Available
> **Surface**: `/feeds` > **Catalog submode**: `/feeds?mode=catalog` > **Compatibility redirect**: `/`
...
- **Reader default** opens the article workspace on `/feeds`.
- **Catalog mode** lives at `/feeds?mode=catalog` when you want to
...
`/` redirects into the same workspace for compatibility, but new docs and links should point to `/feeds` unless backward compatibility is the point.
...
```mermaid
flowchart LR
    catalog["feeds.enriched.yaml / feeds.yaml"] --> route["/feeds"]
