...
<Tabs items={['Node.js (Default)', 'Bun (Optional)']}>
  <Tab value="Node.js (Default)">
    The `scripts/lint-node.mjs` file runs with tsx/Node.js and scans `content/docs`
    directly, so it does not depend on a generated `.map` manifest.

    At a high level, the Node.js path now:

    1. Resolves `content/docs` relative to `import.meta.url`
    2. Recursively collects `.mdx` files from the docs tree
    3. Derives page slugs and heading hashes from the raw MDX files
    4. Passes those pages to `scanURLs()` and `validateFiles()`

    See `apps/web/scripts/lint-node.mjs` for the full implementation.
