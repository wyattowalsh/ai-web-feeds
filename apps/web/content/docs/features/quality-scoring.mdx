...
- **Author Authority**: Author credentials and expertise (partial)
- **Domain Reputation**: Feed source quality and reliability
- **Engagement**: Read time estimates and user signals (partial)
...
manager.start()  # registers feed, trending, digest, and NLP batch jobs
```
...
```sql
ALTER TABLE articles ADD COLUMN quality_processed BOOLEAN DEFAULT FALSE;
ALTER TABLE articles ADD COLUMN quality_processed_at DATETIME;
```
...
**Symptom**: `ai-web-feeds nlp stats` shows 0 quality processed.
...
ai-web-feeds nlp quality --batch-size 10
```
