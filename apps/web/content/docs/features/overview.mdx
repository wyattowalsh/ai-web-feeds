...
description: Reader-first feeds workspace plus catalog, export, analytics, and integration capabilities
icon: Sparkles
---

import { Card, Cards } from "fumadocs-ui/components/card";

ai-web-feeds is organized around one primary product surface: the reader-first
Feeds workspace. Everything else in the product supports that core workflow by
helping you curate sources, refresh the corpus, inspect results, and export the
final set.

## Core Capabilities

<Cards>
  <Card title="Reader-First Feeds" description="Corpus-backed browsing, filtering, preview, and local reading state in /feeds" href="/docs/features/reader" />
...
2. Narrow the source slice from catalog mode when needed.
3. Refresh the slice for live-only additions without replacing the base corpus.
4. Export the curated feed set directly from `/feeds` when it is ready to move elsewhere.
...
- [ ] Reader pagination and background refresh
