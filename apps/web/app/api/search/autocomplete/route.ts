...
export async function GET(request: Request) {
  ...
  try {
    ...
    const mockFeeds = [
      ...
      { id: 'huggingface', title: 'Hugging Face Blog', type: 'feed', url: 'https://huggingface.co/blog' },
      { id: 'pytorch', title: 'PyTorch Blog', type: 'feed', url: 'https://pytorch.org/blog' },
      { id: 'tensorflow', title: 'TensorFlow Blog', type: 'feed', url: 'https://blog.tensorflow.org' },
    ...
    const feeds = mockFeeds
      .filter((f) => f.title.toLowerCase().includes(lowerPrefix))
      .slice(0, 5);
