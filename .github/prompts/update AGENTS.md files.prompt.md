---
mode: agent
description: Comprehensive prompt for systematically updating all AGENTS.md files across the AIWebFeeds monorepo to create a unified, navigable, LLM-optimized documentation system
---
...
**Prerequisites**: Understanding of project structure, markdown, and documentation best practices

---

## Objective

Systematically audit, refactor, and enhance all `AGENTS.md` files across the AIWebFeeds monorepo to establish a cohesive, navigable, and LLM-optimized documentation system that serves as the primary reference for AI coding agents.

**Goal**: Transform disparate AGENTS.md files into an interconnected knowledge graph that enables efficient agent navigation, clear component understanding, and actionable development guidance.

## Context

**Project**: AIWebFeeds - Hybrid monorepo combining Python (uv) and TypeScript (pnpm) for RSS/Atom feed management

**Architecture**: Documentation-first, component-isolated design with LLM-optimized formats
...
**Current State**: Files exist but lack consistency, navigation structure, and proper cross-referencing

**Desired State**: Unified documentation system with clear hierarchy, bidirectional links, and DRY principle adherence
...
- **Critical Patterns**: Never create standalone `.md` files, always use FumaDocs structure
...
- **Critical Patterns**: First file agents should read, links to all other AGENTS.md files
...
2. **Consistency First**: All files MUST follow identical template structure
3. **Brevity Over Completeness**: Prefer bullet points and checklists over paragraphs
4. **DRY Principle**: Reference `llms-full.txt` instead of duplicating detailed content
5. **Zero Markdown Lint Errors**: Follow markdown best practices strictly
6. **Line Length**: Maximum 100 characters per line for readability and git diff clarity
7. **Code Block Fencing**: Always specify language (python, typescript, bash, json, yaml, etc.)
8. **No Orphaned Content**: Every assertion must cite a source (code example, reference link, or pattern)
9. **Version Consistency**: All files must show same version in footer
10. **Accessibility**: Use semantic markdown (proper heading hierarchy, alt text for conceptual diagrams)

## Success Criteria

✅ **Navigation Efficiency**: Can navigate from root to any component AGENTS.md in exactly 1 click
✅ **Three-Question Test**: Each file clearly answers:
   - "What is this component?"
   - "How do I work with it correctly?"
   - "Where do I learn more details?"
✅ **Structural Consistency**: All files use identical section structure and emoji headers
✅ **Reference Integrity**: All `llms-full.txt` links functional with proper section anchors
✅ **Length Compliance**: Component files ≤200 lines, root ≤250 lines
✅ **Actionability**: Every section provides concrete patterns, checklists, or decision trees
✅ **Bidirectional Links**: Parent ↔ child and component ↔ component links work both ways
✅ **Zero Duplication**: Detailed docs live exclusively in `llms-full.txt` or `apps/web/content/docs/`
✅ **Code Quality**: All code examples show both ✅ correct AND ❌ incorrect patterns
✅ **Searchability**: Key terms and patterns are easy to grep/search across all files

## Deliverables

1. Updated `/AGENTS.md` (root navigation hub)
2. Updated `/packages/ai_web_feeds/AGENTS.md`
3. Updated `/apps/cli/AGENTS.md`
4. Updated `/apps/web/AGENTS.md`
5. Updated `/tests/AGENTS.md`
6. Updated `/.github/AGENTS.md`
...
2. [ ] Navigation table in root links to all 5 component AGENTS.md files
3. [ ] Each component file links back to root in "Essential Links" section
4. [ ] All `llms-full.txt` links include proper section anchors
5. [ ] Emoji section headers are consistent across all files (📍 🎯 🏗️ 📐 🧪 🔄 🚨 📚)

### Content Validation
6. [ ] Code examples demonstrate both ✅ correct AND ❌ incorrect patterns
7. [ ] DO/DON'T lists contain 5-7 actionable items each
8. [ ] No markdown lint errors in any file
9. [ ] File lengths comply: component ≤200 lines, root ≤250 lines
10. [ ] Version footer matches exactly: `*Updated: October 2025 · Version: 0.1.0*`

### Link Validation
11. [ ] All relative paths (../../) resolve correctly
12. [ ] All absolute URLs (https://) are reachable
13. [ ] Web component uses `file:///absolute/path` references
14. [ ] Cross-references between components work bidirectionally

### Quality Validation
15. [ ] Each file passes the "Three-Question Test" (What/How/Where)
16. [ ] No duplicated content between AGENTS.md and llms-full.txt
17. [ ] Architecture diagrams use ASCII trees for simplicity
18. [ ] Testing sections link to appropriate test files/docs
19. [ ] Common tasks use numbered checklists, not paragraphs
20. [ ] Reference sections include 3-5 curated external links

## Notes

### Best Practices for Execution

1. **Read First, Write Second**: Before editing any file, read the existing content to understand project-specific conventions
2. **AI Agent Usability > Human Aesthetics**: Optimize for parsing, not presentation
3. **Assumed Reading Order**: Agents will read `root AGENTS.md` → `component AGENTS.md` → `llms-full.txt` in sequence
4. **Path Resolution**: Use relative paths for internal links (`../../`), absolute URLs for external (`https://`)
5. **When in Doubt**: Favor brevity and referencing over attempting completeness
...
Each component's AGENTS.md should acknowledge these relationships in the "Essential Links" section.
...
2. Identify inconsistencies in structure, terminology, and linking
3. Note any project-specific conventions to preserve
4. Create mental map of component relationships

### Phase 2: Root First (15 minutes)
1. Update `/AGENTS.md` as the primary navigation hub
2. Establish canonical terminology and section structure
3. Create complete component navigation table
4. Define standard workflow and critical rules
5. **Validation**: Can you navigate to any component in 1 click?
...
2. **Tests** (`tests/AGENTS.md`) - Validates core
3. **CLI** (`apps/cli/AGENTS.md`) - Uses core
4. **Web** (`apps/web/AGENTS.md`) - Documents all
5. **GitHub** (`.github/AGENTS.md`) - Minimal updates
...
2. Check for circular references or orphaned content
3. Ensure consistent terminology across all files
4. Confirm no content duplication
5. Test navigation flow: root → component → back to root

### Phase 5: Final Quality Check (10 minutes)
1. Run through all 20 validation checklist items
2. Fix any markdown lint errors
3. Verify code examples show ✅ and ❌ patterns
4. Confirm version footers match
5. Test "Three-Question Test" on each file
...
**See**: [llms-full.txt#fetcher](https://aiwebfeeds.com/llms-full.txt#fetcher) for implementation details
```
