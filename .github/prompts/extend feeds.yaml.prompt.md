...
1. **Quality Over Quantity**: Prioritize authoritative, frequently-updated sources
1. **Platform Diversity**: Include various formats (RSS, Atom, YouTube, Reddit, GitHub,
   etc.)
1. **Global Perspective**: Include non-English sources where valuable

### Target Categories (Priority Order)

1. **Research & Academia**: ArXiv feeds, university labs, research groups
1. **Industry Leaders**: Company blogs (AI labs, MLOps platforms, cloud providers)
1. **Open Source**: GitHub repos, foundation blogs, community projects
1. **Technical Blogs**: Individual researchers, engineers, practitioners
1. **News & Analysis**: Trade publications, industry newsletters
1. **Educational Content**: Tutorials, courses, documentation sites
1. **Community Forums**: Subreddits, Discourse forums, Stack Exchange
1. **Podcasts & Media**: Video channels, podcast feeds, webinar series
...
1. **Identify underrepresented topics** from `topics.yaml`
1. **Note missing perspectives** (geographic, organizational, skill-level)
1. **List specific search targets** (e.g., "NLP researcher blogs", "MLOps company
   feeds")
...
1. **Verify feed accessibility** (HTTP 200, valid XML/JSON)
1. **Confirm platform compatibility** (can system auto-discover feed?)
1. **Test topic assignments** (1-6 topics, all from `topics.yaml`)
...
1. **Primary topic first**: Most specific, relevant topic ID
1. **Breadth over depth**: Cover multiple facets (e.g.,
   `[llm, evaluation, open-source]`)
1. **Facet diversity**: Mix domain, task, methodology, format topics
1. **Respect limits**: 1-6 topics (optimal: 2-4)
...
1. ❌ **Using invalid topic IDs** → Cross-reference `topics.yaml`
1. ❌ **Exceeding 6 topics per feed** → Schema violation
1. ❌ **Malformed URLs** → Must be valid HTTP(S)
1. ❌ **Generic/vague notes** → Be specific about value
1. ❌ **Ignoring platform auto-discovery** → Use homepage URLs when possible
1. ❌ **Adding inactive feeds** → Check recent post dates
1. ❌ **Missing required fields** → URL and topics are mandatory
