...
2. Identify likely duplicates by feed id, title, url, or obvious overlap with an
   existing source.
3. Normalize the submission into a concise maintainer-facing summary.
4. Recommend only repository-approved labels.
...
- Do not recommend direct repository edits or pull-request creation in this
  workflow.
...
2. `Normalized Summary`: feed id, title, source type, primary urls, topics,
   mediums
3. `Missing or Ambiguous Fields`: concrete gaps only
4. `Duplicate Concerns`: exact or likely conflicts only
5. `Recommended Labels`: labels from `.github/labels.yml` only
6. `Maintainer Note`: a short operational recommendation
