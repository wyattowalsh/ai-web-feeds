...
def generate_rsshub_url(url: str, platform: str, rsshub_instance: str = "https://rsshub.app") -> str | None:
    ...
    try:
        ...
        if platform == "twitter":
            if match := re.match(r"([A-Za-z0-9_]{1,15})(?:/.*)?$", path):
                ...
                if username.lower() not in ["home", "explore", "notifications", "messages", "i", "settings", "search"]:
...
def generate_reddit_feed_url(url: str, platform_config: dict[str, Any] | None = None) -> str | None:
    ...
    try:
        ...
        if match := re.match(r"r/([A-Za-z0-9_]{3,21})", path):
            ...
            sort = platform_config.get("reddit", {}).get("sort", "hot") if platform_config else "hot"
            time = platform_config.get("reddit", {}).get("time", "all") if platform_config else "all"
        ...
        if platform_config and "reddit" in platform_config:
            ...
            if subreddit := reddit_cfg.get("subreddit"):
                ...
                if sort == "top":
                    ...
                    return f"{base_url}/{sort}/.rss"
            elif username := reddit_cfg.get("username"):
                return f"https://www.reddit.com/user/{username}/.rss"
    ...
    try:
        ...
        if platform_config and "medium" in platform_config:
            ...
            if publication := medium_cfg.get("publication"):
                return f"https://medium.com/feed/{publication}"
            elif username := medium_cfg.get("username"):
                username = username.lstrip("@")
                return f"https://medium.com/feed/@{username}"
            elif tag := medium_cfg.get("tag"):
                return f"https://medium.com/feed/tag/{tag}"
...
def generate_youtube_feed_url(url: str, platform_config: dict[str, Any] | None = None) -> str | None:
    ...
    try:
        ...
        if platform_config and "youtube" in platform_config:
            ...
            if channel_id := yt_cfg.get("channel_id"):
                return f"https://www.youtube.com/feeds/videos.xml?channel_id={channel_id}"
            elif playlist_id := yt_cfg.get("playlist_id"):
                return f"https://www.youtube.com/feeds/videos.xml?playlist_id={playlist_id}"
            elif username := yt_cfg.get("username"):
                return f"https://www.youtube.com/feeds/videos.xml?user={username}"
...
def generate_substack_feed_url(url: str, platform_config: dict[str, Any] | None = None) -> str | None:
...
def generate_devto_feed_url(url: str, platform_config: dict[str, Any] | None = None) -> str | None:
    ...
    try:
        ...
        if platform_config and "devto" in platform_config:
            ...
            elif org := devto_cfg.get("organization"):
                return f"https://dev.to/feed/{org}"
            elif tag := devto_cfg.get("tag"):
                return f"https://dev.to/feed/tag/{tag}"
...
def generate_hackernews_feed_url(url: str, platform_config: dict[str, Any] | None = None) -> str | None:
...
def generate_twitter_feed_url(url: str, platform_config: dict[str, Any] | None = None) -> str | None:
...
def generate_platform_feed_url(url: str, platform: str, platform_config: dict[str, Any] | None = None) -> str | None:
...
async def discover_feed_url(site_url: str, platform_config: dict[str, Any] | None = None) -> str | None:
            ...
            for feed_url in common_feeds:
                try:
                    ...
                    if feed_resp.status_code == 200:
                        ...
                        if any(
                            t in content_type
                            for t in ["xml", "rss", "atom", "feed", "json"]
                        ):
...
async def enrich_feed_source(
    ...
    if any(pattern in input_url.lower() for pattern in ['/feed', '/rss', '/atom', '.xml', '.rss', '.atom']):
        feed_url = input_url
    ...
    if feed_url:
        ...
        enriched["site"] = input_url if enriched["meta"].get("input_type") != "direct_feed" else None
    else:
...
def generate_opml(
    feed_sources: list[FeedSource], title: str = "AI Web Feeds"
) -> str:
    ...
    SubElement(head, "dateCreated").text = datetime.utcnow().strftime(
        "%a, %d %b %Y %H:%M:%S GMT"
    )
    SubElement(head, "dateModified").text = datetime.utcnow().strftime(
        "%a, %d %b %Y %H:%M:%S GMT"
    )
