...
class QualityScorer:
    ...
    def score_article(self, article: dict, feed: Optional[dict] = None) -> Optional[QualityScoreComponents]:
        ...
        try:
            ...
            if word_count < self.min_words:
                logger.debug(f"Article too short ({word_count} words < {self.min_words}), skipping quality scoring")
                return None
    ...
    def _score_references(self, content: str, article_url: str) -> int:
        ...
        quality_count = sum(1 for url in external_urls if any(domain in url for domain in quality_domains))
        quality_score = min(30, quality_count * 10)
        
        return min(100, link_score + quality_score)
    
    def _score_author(self, author: Optional[str], author_detail: Optional[dict]) -> int:
    ...
    def _score_domain(self, feed: Optional[dict]) -> int:
