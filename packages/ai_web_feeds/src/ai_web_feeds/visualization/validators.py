...
MAX_CUSTOMIZATION_TITLE_LENGTH = 100
...
def validate_table_name(table_name: str) -> str:
    """Compatibility wrapper for table whitelist validation."""
    return QueryValidator.validate_table_name(table_name)


def validate_query_limit(limit: int | None, max_limit: int = 10_000) -> int:
    """Compatibility wrapper that caps limits instead of rejecting large values."""
...
def validate_date_range(
    ...
    """Validate ISO date range strings."""
    try:
    ...
    if start_date > today or end_date > today:
        raise ValidationError("Date range cannot include future dates")
...
def sanitize_like_clause(value: str) -> str:
    """Escape LIKE wildcards while preserving literal text."""
    return value.replace("'", "''").replace("%", r"\%").replace("_", r"\_")


def validate_dashboard_constraints(widgets: list[dict[str, int]]) -> None:
    """Compatibility wrapper for dashboard widget layout validation."""
    if len(widgets) > MAX_WIDGETS_PER_DASHBOARD:
        raise ValidationError(f"Dashboard supports a maximum 20 widgets (got {len(widgets)})")
    ...
    for widget in widgets:
        ...
        try:
            ...
            raise ValidationError(f"Widget out of bounds: {exc}") from exc
        normalized_positions.append(position)

    DashboardValidator.check_widget_overlap(normalized_positions)


def validate_customization(customization: dict[str, Any]) -> dict[str, Any]:
    """Compatibility wrapper for chart customization validation."""
    title = customization.get("title")
    if title is not None and len(title) > MAX_CUSTOMIZATION_TITLE_LENGTH:
        raise ValidationError("Chart title must be 100 characters or fewer")

    colors = customization.get("colors")
    if colors is not None:
        CustomizationValidator.validate_colors(colors)

    legend_position = customization.get("legend_position")
    if legend_position is not None and legend_position not in {"top", "bottom", "left", "right"}:
        raise ValidationError("Invalid legend position")

    font_size = customization.get("title_font_size")
    if font_size is not None:
        CustomizationValidator.validate_font_size(font_size)
...
def validate_forecast_data(
    ...
    """Compatibility wrapper for forecast response validation."""
    if len(predictions) != horizon_days:
        raise ValidationError("Forecast predictions count does not match horizon")

    required_fields = {"date", "value", "lower", "upper"}
    parsed_dates: list[date] = []
    for prediction in predictions:
        missing_fields = required_fields.difference(prediction)
        if missing_fields:
            raise ValidationError("Forecast entries must include all required fields")
        if prediction["lower"] > prediction["upper"]:
            raise ValidationError("Forecast lower bound cannot exceed upper bound")

        try:
            parsed_dates.append(date.fromisoformat(str(prediction["date"])))
        except ValueError as exc:
