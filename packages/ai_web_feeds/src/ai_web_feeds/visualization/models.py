...
class Dashboard(SQLModel, table=True):
    ...
    @classmethod
    def _normalize_legacy_dashboard_payload(cls, data: dict[str, Any]) -> dict[str, Any]:
        normalized = dict(data)
        if "layout_config" in normalized and "layout" not in normalized:
            normalized["layout"] = normalized.pop("layout_config")
        return normalized

    def __init__(self, **data: Any):
        super().__init__(**self._normalize_legacy_dashboard_payload(data))

    @property
    def layout_config(self) -> dict[str, Any]:
        """Legacy alias for the dashboard layout payload."""
        return self.layout
...
class Forecast(SQLModel, table=True):
    ...
    data_source: DataSource | None = SQLField(default=None)
    topic_id: int | None = SQLField(
    ...
    @classmethod
    def _normalize_legacy_forecast_payload(cls, data: dict[str, Any]) -> dict[str, Any]:
        normalized = dict(data)
        if "horizon_days" in normalized and "forecast_horizon_days" not in normalized:
            normalized["forecast_horizon_days"] = normalized.pop("horizon_days")
        if "metrics" in normalized and "accuracy_metrics" not in normalized:
            normalized["accuracy_metrics"] = normalized.pop("metrics")
        return normalized

    def __init__(self, **data: Any):
        super().__init__(**self._normalize_legacy_forecast_payload(data))

    @property
    def horizon_days(self) -> int:
        return self.forecast_horizon_days

    @horizon_days.setter
    def horizon_days(self, value: int) -> None:
        self.forecast_horizon_days = value
